    ...
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Secret()
        {
            var serverResponse = await AccessTokenRefreshWrapper(() => 
                SecureGetRequest("https://localhost:44324/secret/index"));

            var apiResoponse = await AccessTokenRefreshWrapper(() =>
                SecureGetRequest("https://localhost:44391/secret/index"));
        ...
        {
            var requestResponse = await InitialRequest();
            if (requestResponse.StatusCode == HttpStatusCode.Unauthorized)
            {
                await RefreshAccessToken();
                requestResponse = await InitialRequest();
            }
        ...
        private async Task<HttpResponseMessage> RefreshAccessToken()
        {
            ...
            var request = new HttpRequestMessage(HttpMethod.Post, "https://localhost:44324/oauth/token")
            {
